...
import { X, Package, Truck, CheckCircle2, Clock, AlertCircle, MapPin, Phone, Mail } from 'lucide-react'
...
const OrderHistory = () => {
  ...
  // Handle the response structure - could be { orders: [] } or just []
  const ordersList = orders?.orders || orders || []
  
  // Calculate item count for each order (we'll fetch full details in modal)
  const getItemCount = (order) => {
    // Priority: use item_count from API if available
    if (order.item_count !== undefined && order.item_count !== null) {
      return order.item_count
    }
    // Fallback to counting items if available
          ...
          {ordersList.length === 0 ? (
            ...
            <div className="space-y-4">
              {ordersList.map((order) => {
                ...
                return (
                  <motion.div
                    key={order.id || order.resource_id}
                  ...
                  >
                    <div className="flex items-start justify-between mb-4">
                      ...
                      <div className="text-right">
                        <p className="font-bold text-lg text-gray-900">{formatPrice(order.total || order.total_amount || 0)}</p>
